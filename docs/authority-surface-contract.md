...
- `implementation-status` routed to the obsolete PR #55-only / Issue #56 snapshot after the authority chain advanced through PR #69;
- current implementation status that omits the sibling artifact-identity lane or collapses open Issue into absent implementation;
...
The self-test plants representative stale claims across all six routing surfaces and requires every mutation to fail. The current suite contains 25 planted drifts, including five implementation-status routing regressions.

## Ownership split

- `docs/implementation-status.md`: current merged/staged engineering, exact-head evidence routing, open-Issue engineering semantics, and remaining admission gates.
...
When an intentional architecture, licensing, platform, product, branch, or evidence transition makes an assertion obsolete, update the owning implementation/evidence first, then reconcile every affected routing surface and its planted controls. Never weaken the gate merely to make CI green.
