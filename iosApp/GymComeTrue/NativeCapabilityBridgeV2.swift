...
final class NativeLabelEvidenceBridgeV2 {
    func scan(
        image: UIImage,
        completion: @escaping (Result<NativeScanEvidenceV2, Error>) -> Void
    ) {
        guard let cgImage = image.cgImage else {
            completion(.failure(NativeScanErrorV2.missingCGImage))
            return
        }
            ...
            autoreleasepool {
                do {
                    ...
                    guard !text.isEmpty || barcode != nil else {
                        throw NativeScanErrorV2.noRecognizedContent
                    }

                    let digest = SHA256.hash(data: Data(text.utf8))
                        .map { String(format: "%02x", $0) }
                        .joined()
                    let evidence = NativeScanEvidenceV2(
                        recognizedText: text,
...
final class NativeReminderBridgeV2 {
